"""
OpenAI-only, guarded extractor implementing a 5-step pipeline (optimized) with progress prints:
...
class _Logger:
    ...
    def error(self, msg: str):
        with self._lock:
            print(f"[{_now()}] ERROR: {msg}", flush=True)
...
def _call_json_with_retries(call_fn, schema_model=None, retries=3, sleep=1.5, label: str = ""):
    last_exc = None
    for i in range(retries):
        try:
            ...
            if label:
                log.note(f"{label}: parse ✓")
            return obj
        except Exception as e:
            last_exc = e
            # exponential backoff with light jitter
            delay = sleep * (2 ** i) * (0.85 + 0.3 * random.random())
            if label:
                log.note(f"{label}: retry {i+1}/{retries} in {delay:.1f}s ({type(e).__name__})")
            time.sleep(delay)
    raise last_exc
...
class OpenAIGuardedExtractor:
    ...
    def extract_sections_from_pdf(self, pdf_path: str, max_chars: int = 4000) -> List[Tuple[str, str]]:
        ...
        """
        log.step(f"PDF sectioning: {pdf_path}")
        raw: List[Tuple[int, str]] = []
        with open(pdf_path, "rb") as f:
            reader = pypdf.PdfReader(f)
            log.note(f"Pages found: {len(reader.pages)}")
        ...
        log.note(f"Clinical pages kept: {kept} → Chunks: {len(chunks)}")
        return chunks
    ...
    def _chat_json(self, system: str, user: str, max_tokens: int = 6000, model_key: str = "section", schema_model=None) -> Dict[str, Any]:
        def _call():
            ...
            log.note(f"[LLM] {model_key} → {model} (max_tokens={max_tokens})")
            resp = self.client.chat.completions.create(
                ...
                seed=self.seed,  # supported; safe if ignored by SDK version
            )
            return (resp.choices[0].message.content or "").strip()

        try:
            return _call_json_with_retries(_call, schema_model=schema_model, retries=3, sleep=1.5, label=model_key)
        except Exception as e:
            Path(self.debug_dir / "guarded_debug_last.txt").write_text(f"SYSTEM:\n{system}\n\nUSER:\n{user}\n\nERR:{e}", encoding="utf-8")
            log.error(f"{model_key} failed: {e}")
            raise
    ...
    def extract_rules_per_section(self, pdf_path: str) -> List[Dict[str, Any]]:
        ...
        log.step(f"Step 1/5: Per-section extraction → JSON IR (sections={len(sections)})")
        results: List[Dict[str, Any]] = []
        total = len(sections)
        for idx, (sec_id, text) in enumerate(sections, start=1):
            ...
            try:
                ...
                log.note(f"Section {sec_id}: ✗ ({idx}/{total}) reason={type(e).__name__}")
                continue
            ...
            results.append(obj)
            log.note(f"Section {sec_id}: ✓ rules={len(obj['rules'])} ({idx}/{total})")

        log.note(f"Per-section complete: {len(results)} ok / {total} total")
        return results

    # ---------------- Step 2: merge -----------------
    def merge_sections(self, section_objs: List[Dict[str, Any]]) -> Dict[str, Any]:
        log.step("Step 2/5: Global merge + canonicalization")
        ...
        try:
            ...
            log.note("Falling back to deterministic merge")
        ...
        log.note(f"Merged IR: vars={len(merged.get('variables', []))}, rules={len(merged.get('rules', []))}")
        return merged

    # ---------------- Step 3: DMN + ASK_PLAN -----------------
    def generate_dmn_and_ask_plan(self, merged_ir: Dict[str, Any]) -> Tuple[str, Any]:
        log.step("Step 3/5: DMN (FIRST) + ASK_PLAN")
        ...
        try:
            ...
            log.note(f"Wrote raw DMN/ASK response → {self.debug_dir/'dmn_ask_debug_last.txt'}")
        except Exception:
            pass
        ...
        if not dmn_xml or ask_plan is None:
            try:
                ...
                pass
            log.error("DMN/ASK_PLAN missing from LLM output")
        ...
        try:
            ...
            if unknowns:
                ...
                log.note(f"ASK_PLAN unknowns dropped: {sorted(list(unknowns))}")
        ...
        try:
            (self.debug_dir / "dmn.xml").write_text(dmn_xml, encoding="utf-8")
            (self.debug_dir / "ask_plan.json").write_text(json.dumps(ask_plan, indent=2), encoding="utf-8")
            log.note(f"Wrote: {self.debug_dir/'dmn.xml'}; {self.debug_dir/'ask_plan.json'}")
        except Exception:
            pass
    ...
    def generate_bpmn(self, dmn_xml: str, ask_plan: Any) -> str:
        log.step("Step 4/5: BPMN (from DMN + ASK_PLAN)")
        ...
        log.error("BPMN not found in response")
        raise RuntimeError("Failed to get BPMN from model")

    # ---------------- Step 5: coverage -----------------
    def audit_coverage(self, merged_ir: Dict[str, Any], dmn_xml: str) -> Dict[str, Any]:
        log.step("Step 5/5: Coverage audit (RULES_JSON → DMN rows)")
        payload = {
            "ir": merged_ir,
            "ir_flat": _rules_flattened(merged_ir)  # helper for exact literal matching
        }
        user = "RULES_JSON:\n" + json.dumps(payload, ensure_ascii=False) + "\nDMN:\n```xml\n" + dmn_xml + "\n```"
        out = self._chat_json(self.coverage_system, user, max_tokens=6000, model_key="audit", schema_model=None)
        try:
            (self.debug_dir / "coverage.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
            log.note(f"Wrote: {self.debug_dir/'coverage.json'}")
        except Exception:
            pass
        unmapped = out.get("unmapped_rule_ids") or []
        log.note(f"Coverage: unmapped rules = {len(unmapped)}")
        return out

    # ---------------- Convenience: run all steps -----------------
    def run_pipeline(self, pdf_path: str) -> Dict[str, Any]:
        """
        Convenience wrapper to run the full 5-step pipeline with progress prints.
        Returns a dict containing section IRs, merged IR, DMN XML, ASK_PLAN, BPMN XML, and coverage JSON.
        """
        sections = self.extract_rules_per_section(pdf_path)
        merged = self.merge_sections(sections)
        dmn_xml, ask_plan = self.generate_dmn_and_ask_plan(merged)
        bpmn_xml = self.generate_bpmn(dmn_xml, ask_plan)
        coverage = self.audit_coverage(merged, dmn_xml)

        log.step("Pipeline complete ✅")
        log.note(f"Summary: sections_ok={len(sections)}, vars={len(merged.get('variables', []))}, rules={len(merged.get('rules', []))}, "
                 f"ask_modules={len(ask_plan) if isinstance(ask_plan, list) else 'n/a'}, "
                 f"coverage_unmapped={len(coverage.get('unmapped_rule_ids') or [])}")
        return {
            "sections": sections,
            "merged_ir": merged,
            "dmn_xml": dmn_xml,
