...
"""
End to end pipeline for turning a WHO style guideline PDF into:
- section level JSON (variables + rules)
- a merged intermediate representation (IR)
- DMN decision tables
- a BPMN wrapper
- a CHT orchestrator folder (XLSForm + media + props).

The design goal is that someone can read this file top to bottom and
see every step of the transformation without chasing imports.
"""
...
def _text_or(node, default: str = "") -> str:
    """
    Return node.text stripped, or a default if the node or text is missing.
    Safe helper for xml.etree lookups.
    """
    if node is None:
        return default
...
def _strip_quotes(s: str) -> str:
    """Strip a single leading and trailing quote pair if present."""
    if s is None:
        return ""
    ss = str(s).strip()
...
def _and_join(parts: List[str]) -> str:
    """
    Join a list of conditions with logical 'and', skipping empty parts.
    Used when translating FEEL conditions into XLSForm relevant clauses.
    """
...
def _validate_dmn_outputs_or_die(dmn_xml: Optional[str]) -> None:
    """
    Cheap guard to avoid crashes later in the pipeline.

    Only fail hard if DMN is completely missing. This lets us run coverage
    and orchestrator export in tests without tightly validating DMN content.
    """
...
def _ensure_dmn_inputs(dmn_xml: Optional[str], ask_plan: Any, merged_ir: Dict[str, Any]) -> str:
    """
    Best effort no-op fixer for DMN input definitions.

    Right now this simply returns the DMN unchanged if present, or a minimal
    stub if DMN was empty. You can extend this to inject missing inputs based
    on ASK_PLAN and merged_ir if you want stronger guarantees.
    """
    if not dmn_xml:
...
def _extract_pypdf_pages(pdf_path: str) -> List[Tuple[str, str]]:
    ...
    Returns a list of (page_id, text), where page_id is 'p1', 'p2', etc.
    On failure, returns an empty list so callers can fall back to other
    backends (pdfminer or OCR).
    """
...
def _postprocess_page_text(pages: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    """
    Light cleanup pass on raw page text.
...
def _chunk_pages_len_only(pages: List[Tuple[str, str]], max_chars: int = 4000) -> List[Tuple[str, str]]:
    """
    Fallback chunking by character count only, ignoring headings.

    Creates 'sec_p1_p2' style ids that note which pages are bundled.
    """
    ...
    for pid, txt in pages:
        if cur_len + len(txt) > max_chars and buf:
            chunks.append(
                (
                    "sec_" + "_".join(cur_id[:2]) + ("" if len(cur_id) < 3 else f"_{len(cur_id)}"),
                    "\n".join(buf),
                )
            )
...
def _split_into_sections_by_headings(pages: List[Tuple[str, str]], max_chars: int = 4000) -> List[Tuple[str, str]]:
    """
    Heuristic sectioner: start a new section when we see an ALL CAPS line
    or a line that looks like a numbered heading (1, 1.1, 2.3.4 etc).

    If the heuristic finds nothing, fall back to length based chunking.
    """
    ...
    for pid, txt in pages:
        ...
        for ln in lines:
            is_head = bool(re.match(r"^\s*(\d+(\.\d+)*)\s+\S", ln)) or (
                ln.strip().isupper() and len(ln.strip()) >= 6
            )
...
def _extract_pdfminer_layout_pages(pdf_path: str) -> List[Tuple[str, str]]:
    """
    Placeholder for richer pdfminer layout extraction if needed later.

    Right now we prefer pypdf for simplicity; pdfminer hooks live here
    so the call sites do not have to change when we upgrade.
    """
    return []
...
def _extract_ocr_pages(pdf_path: str) -> List[Tuple[str, str]]:
    """
    Placeholder for OCR fallback (pytesseract + pdf2image).

    We keep this stubbed so callers can try OCR if both pypdf and
    pdfminer fail for a given document.
    """
    return []
...
if PydanticAvailable:
    # These models define the canonical IR schema used across the pipeline.
    # They are intentionally minimal so the JSON interchange format is stable.

    Op = Union[str]

    class Variable(BaseModel):
        """A single variable that the CHW app can ask for or compute."""
        name: str
        type: str
    ...
    class ObsCond(BaseModel):
        """Numeric or categorical comparison on an observation."""
        obs: str
        op: str
        value: Union[float, int, bool, str]

    class SymCond(BaseModel):
        """Presence or absence of a symptom or flag."""
        sym: str
        eq: bool

    class AllOf(BaseModel):
        """Logical AND of a list of conditions."""
        all_of: conlist(Union[ObsCond, SymCond], min_length=1)

    class AnyOf(BaseModel):
        """Logical OR of a list of conditions."""
    ...
    class ThenBlock(BaseModel):
        """
        Outputs of a rule: triage plus any flags, reasons, and actions.

        actions includes CHW treatments and tasks that the module should
        propose (for example give_ors, give_zinc, teach_handwashing).
        """
    ...
    class Rule(BaseModel):
        """A single decision rule in the IR."""
        rule_id: str
        when: conlist(Condition, min_length=1)
        then: ThenBlock

    class QA(BaseModel):
        """Debug and QA metadata about the IR."""
        notes: List[str] = []
    ...
    class IR(BaseModel):
        """
        Intermediate representation of one guideline:
        - variables: all questions and derived values
        - rules: classification and triage rules
        - canonical_map: mapping from synonyms to canonical variable names
        - qa: notes for audits and debugging
        """
...
def _strip_code_fences(text: str) -> str:
    """
    Strip a single top level ``` block from model output.

    We do not try to be clever here; this only handles the common pattern
    where the entire response is fenced.
    """
    if not text:
        return text
    s = text.strip()
...
def _loose_to_json(s: str) -> Any:
    """
    Parse slightly messy JSON emitted by the model into Python objects.

    Handles:
    - code fences
    - C style block comments and line comments
    - trailing commas before closing ] or }
    - stray text before or after the main object or array.
    """
...
def _call_json_with_retries(call_fn, schema_model=None, retries: int = 3, sleep: float = 1.5):
    """
    Call a function that returns JSON like text, parse it, and retry on failure.

    - call_fn: zero arg function that returns a raw string (model content)
    - schema_model: optional pydantic model that will validate and normalize
      the parsed JSON
...
def _extract_fenced_blocks(text: str) -> List[Tuple[str, str]]:
    """
    Extract all ```lang ... ``` blocks from a string.

    Returns a list of (language_hint, body_text) pairs.
    """
...
def _extract_xml_tag(raw: str, tag: str) -> Optional[str]:
    """
    Extract a single top level XML tag block from a string.

    The tag parameter should include the namespace if present, for example
    'bpmn:definitions'.
    """
...
def _sanitize_bpmn(xml: str) -> str:
    """
    Very light sanitation for BPMN XML so downstream tools do not choke.

    - ensure the bpmn namespace is declared
    - ensure there is a closing </bpmn:definitions> tag
    """
    s = (xml or "").strip()
    if not s:
        return s
...
def _to_snake(name: str) -> str:
    """Normalize a free text label to snake_case for internal use."""
...
def _norm_name(s: Optional[str]) -> str:
    """Strip a possibly None string and return a safe empty string on None."""
    return (s or "").strip()
...
def _clean_bool(v: Any) -> Optional[bool]:
    """Best effort conversion of various truthy strings into a bool."""
...
def _clean_number_or_str(v: Any) -> Union[float, int, str, bool]:
    """
    Try to interpret a value as int, float, or bool; fall back to string.

    This lets us keep model output flexible while still making comparisons.
    """
...
def _map_op(op: str) -> Optional[str]:
    """Map various operator spellings into the internal small set."""
...
def _first_key(d: Dict[str, Any], keys: set) -> Optional[str]:
    """Return the first actual dict key whose snake version is in keys."""
...
def _unwrap_cond_wrappers(c: Any) -> Any:
    """
    Strip one level of wrapper like SymCond, ObsCond, AnyOf, AllOf, etc.

    Model output often nests conditions under these keys, so we unwrap them
    to a plain dict before normalizing.
    """
...
def _coerce_condition(cond: Any) -> Optional[Dict[str, Any]]:
    """
    Normalize an arbitrary condition into one of three canonical shapes:

    - symptom:
        { "sym": name, "eq": bool }
    - observation:
        { "obs": name, "op": "lt|le|gt|ge|eq|ne", "value": scalar }
    - grouped:
        { "any_of": [COND...] } or { "all_of": [COND...] }

    Returns None if the input cannot be safely interpreted.
    """
    c = _unwrap_cond_wrappers(cond)
    if not isinstance(c, dict):
        return None

    # grouped conditions first
...
def _normalize_rule_schema(rule: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Normalize a raw rule object into the canonical schema.

    - canonicalizes then.triage and then.flags keys
    - fills missing list fields with []
    - normalizes priority to int
    - drops rules that have no valid WHEN conditions
    """
    r = dict(rule or {})
...
def _dedupe_rule_ids(rules: List[Dict[str, Any]], prefix: str = "r") -> List[Dict[str, Any]]:
    """
    Ensure rule_ids are unique and stable.

    If a duplicate id appears, suffixes __2, __3, etc are appended.
    """
    seen = {}
    out = []
...
def _build_canonical_map(config: Dict[str, Any], variables: List[Dict[str, Any]]) -> Dict[str, str]:
    """
    Build a mapping from all known synonyms to a canonical variable name.

    Canonical names come from config['canonical_variables'] where possible.
    If a variable has no configured canonical, its own name is used.
    """
...
def _rewrite_rule_to_canon(rule: Dict[str, Any], canon: Dict[str, str]) -> Dict[str, Any]:
    """
    Rewrite sym and obs names inside one rule using the canonical map.

    This keeps rule shape intact while making all variable references
    consistent with the final IR variable list.
    """
    r = dict(rule)
...
def _flatten_rule_conditions(rule: Dict[str, Any]) -> List[str]:
    """
    Flatten WHEN conditions into string expressions for coverage matching.

    These are only used for approximate matching between rules and DMN
    input entries, not for execution.
    """
    out = []
...
def _rules_flattened(ir: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Build a flattened list of rules for coverage prompts.

    Each entry has:
        - rule_id
        - triage level
        - conds: list of stringified conditions
    """
    return [
        {
...
def _preflight_ir(ir: Dict[str, Any]) -> None:
    """
    Sanity checks on the final IR.

    - no duplicate variable names
    - no duplicate rule_ids
    - no rules with empty WHEN clauses
    """
    names = [
...
def _enforce_ask_ownership(
    ...
    """
    Ensure that each variable is only asked by a single module.

    Earlier modules in priority_order have ownership preference. Later
    modules that list the same variable will have it removed from their
    ask and followups_if lists.
    """
    owner: Dict[str, str] = {}
    order = {m: i for i, m in enumerate(priority_order)}
    for blk in sorted(
        [b for b in ask_plan if isinstance(b, dict)],
...
def _pages_from_ref_string(s: str) -> List[int]:
    """
    Parse page references like 'p3' or 'p3-5' into a list of ints.
    """
    if not s:
        return []
    out = []
...
def _pages_from_refs(refs: Optional[List[str]]) -> set:
    """Collect all page numbers mentioned in a list of ref strings."""
    pages = set()
    for r in (refs or []):
...
def _var_sig(v: Dict[str, Any]) -> tuple:
    """
    Variable signature for grouping: type, unit, and allowed values.
    """
    t = v.get("type")
    u = v.get("unit")
...
def _coalesce_group(vars_with_same_sig: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Merge a set of variables that share the same signature into one.

    Synonyms, refs, and allowed values are unioned; prompt is taken from
    the first variable that has one.
    """
...
def _resolve_variables_snake_and_rewrite_rules(merged: Dict[str, Any]) -> Dict[str, Any]:
    """
    Final variable and rule name reconciliation.

    Steps:
    - group variables by snake_case base name
    - within each group, split homonyms by signature and create variants
      (base, base__2, base__3, etc)
    - use page overlap between variables and rules to pick the right
      variant name for each rule condition
    - rewrite conditions in all rules to these final names
    """
    qa_notes = merged.setdefault("qa", {}).setdefault("notes", [])
    vars_in = [
        v
        for v in (merged.get("variables") or [])
        if isinstance(v, dict) and _norm_name(v.get("name"))
    ]
    ...
    for v in vars_in:
        ...
        names_seen_per_group.setdefault(key, set()).update(
            {orig.lower(), *[str(s).strip().lower() for s in syns]}
        )
    ...
    for base, gvars in groups.items():
        ...
        for _sig, same_sig_vars in sig_bins.items():
            ...
            idx += 1
            originals = {
                _norm_name(x.get("name"))
                for x in same_sig_vars
    ...
    def pick_variant(seen_name: str, rule_ref: Optional[str]) -> str:
        """
        Choose the best variable variant for a rule condition.

        If we know the page range for this rule, prefer the variant whose
        refs overlap that range the most. Otherwise use the first variant.
        """
    ...
    if rename_events:
        merged.setdefault("qa", {}).setdefault("notes", []).append(
            f"snake_case_renamed:{rename_events}"
        )
...
def _fact_sheet_from_sections(section_objs: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Combine per section extraction outputs into a single "fact sheet".

    This keeps:
    - variables: merged by exact case sensitive name
    - rules: normalized and deduped by rule_id prefix 'fs'
    """
    ...
    for sec in (section_objs or []):
        for v in (sec.get("variables") or []):
            ...
            if cur is None:
                ...
                cur["synonyms"] = sorted(
                    list(set((cur.get("synonyms") or []) + (v.get("synonyms") or [])))
                )
                cur["refs"] = sorted(
                    list(set((cur.get("refs") or []) + (v.get("refs") or [])))
                )
...
def _canon_cond(cond: Dict[str, Any]) -> Dict[str, Any]:
    """
    Canonical representation of a condition for hashing and grouping.

    All variable names are snake cased and grouped lists are sorted so
    that logically equivalent conditions get the same signature.
    """
    ...
    def canon_one(c: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "obs" in c:
            return {
                "obs": _to_snake(c["obs"]),
                "op": _map_op(c.get("op") or "eq") or "eq",
                "value": _clean_number_or_str(c.get("value")),
            }
        if "all_of" in c or "any_of" in c:
...
def _conds_signature(when: List[Dict[str, Any]]) -> Tuple[str, ...]:
    """
    Hashable signature for a list of conditions.

    Conditions are canonicalized and sorted; we return a tuple of
    JSON strings so it can be used as a dict key.
    """
...
def _merge_then_blocks(blocks: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Merge a list of then blocks that have identical condition signatures.

    triage and guideline_ref are taken from the first block; flags, reasons,
    and actions are unioned in a stable way, and priority is the max.
    """
    if not blocks:
        return {
            "triage": "home",
...
def _index_fact_sheet_rules(fact_sheet: Dict[str, Any]) -> Dict[Tuple[str, ...], List[Dict[str, Any]]]:
    """
    Build an index from condition signatures to original fact sheet rules.

    This is used when we need to preserve triage and ref variants after
    consolidation.
    """
...
def _group_by_triage_and_ref(rules: List[Dict[str, Any]]) -> Dict[Tuple[str, str], List[Dict[str, Any]]]:
    """
    Group rules by (triage, guideline_ref) pair.
    """
...
def _preserve_ref_and_triage_variants(
    cleaned_rules: List[Dict[str, Any]],
    fact_sheet: Dict[str, Any],
) -> Tuple[List[Dict[str, Any]], List[str]]:
    """
    Post processing step after consolidation.

    If the same condition signature appears with multiple (triage, ref)
    combinations in the original fact sheet, split the consolidated rule
    back into per variant copies so behavior is preserved.
    """
    notes: List[str] = []
    ...
    for r in cleaned_rules:
        ...
        for (tri, ref), rs in groups.items():
            ...
            did_split += 1
        notes.append(
            f"ref_triage_variants_preserved:{r.get('rule_id')}->{did_split}"
        )
...
class OpenAIGuardedExtractor:
    """
    High level orchestrator for the extraction pipeline.

    This class handles:
    - calling OpenAI chat completions for section extraction, merge, DMN, BPMN
    - local post processing (lossless rule consolidation, variable resolution)
    - exporting DMN, BPMN, XLSForm, and orchestrator folders.
    """
    ...
    def __init__(
        ...
        if not key and not os.getenv("SKIP_OPENAI_CHECK"):
            # It is valid to construct this class without an API key as
            # long as you only call methods that do not hit the model.
            pass
        ...
        # Debug directory for failures and last prompts
        self.debug_dir = Path(".chatchw_debug")
        self.debug_dir.mkdir(exist_ok=True)

        # Load canonical variable config if present, else default fallbacks.
        default_canon_vars = [
            "symptom1",
        ...
        # System prompts. These are intentionally verbose so that the model
        # is constrained to the IR schema we expect.
    ...
    def _accumulate_usage(self, resp) -> None:
        """
        Accumulate token usage from a chat completion response into self.usage.
        """
        try:
            ...
            t = int(
                ...
                or (u.get("total_tokens") if isinstance(u, dict) else (p + c))
            )
            self.usage["prompt"] += p
    ...
    def get_usage_summary(self) -> Dict[str, int]:
        """Return a shallow copy of the cumulative token usage."""
        return dict(self.usage)

    def _complete(self, model_name: str, messages: list, max_out: int):
        """
        Raw wrapper around client.chat.completions.create with a bit of
        compatibility glue for different model parameter sets.
        """
        if not self.client:
        ...
        try:
            resp = self.client.chat.completions.create(
                max_completion_tokens=max_out, **base_seed, **extras
            )
            ...
            if "verbosity" in msg1 or "reasoning" in msg1 or "unsupported parameter" in msg1:
                try:
                    resp = self.client.chat.completions.create(
                        max_completion_tokens=max_out, **base_seed
                    )
                    self._accumulate_usage(resp)
                    return resp
                    ...
                    if "max_completion_tokens" in msg2 or "unsupported parameter" in msg2:
                        try:
                            ...
                            if "seed" in msg3 or "unsupported parameter" in msg3:
                                ...
                                resp = self.client.chat.completions.create(
                                    max_tokens=max_out, **base_noseed
                                )
                                self._accumulate_usage(resp)
                                return resp
                            raise
            ...
            if "max_completion_tokens" in msg1:
                try:
                    resp = self.client.chat.completions.create(
                        max_tokens=max_out, **base_seed, **extras
                    )
                    self._accumulate_usage(resp)
                    return resp
                    ...
                    if "seed" in msg4 or "unsupported parameter" in msg4:
                        base_noseed = dict(base)
                        resp = self.client.chat.completions.create(
                            max_tokens=max_out, **base_noseed, **extras
                        )
                        self._accumulate_usage(resp)
                        return resp
                    raise
    ...
    def _chat_json(
        ...
        user: str,
        max_out: int = 6000,
        model_key: str = "section",
        schema_model=None,
    ) -> Dict[str, Any]:
        """
        Convenience wrapper for JSON returning chat completions.

        Uses _call_json_with_retries to robustly parse and optionally
        ...
        try:
            return _call_json_with_retries(
                _call, schema_model=schema_model, retries=3
            )
        except Exception as e:
            dbg_user = user
            max_head = 12000
    ...
    def _chat_text(
        ...
        """
        Text returning chat completion helper for DMN and BPMN prompts.
        """
    ...
    def extract_rules_per_section(self, pdf_path: str) -> List[Dict[str, Any]]:
        """
        End to end: PDF pages -> sections -> section level IR objects.

        Each section is processed independently by the model and then
        lightly cleaned and normalized.
        """
        ...
        for sec_id, text in sections:
            ...
            for r in obj.get("rules", []) or []:
                ...
                if nr:
                    ...
                    obj.setdefault("qa", {}).setdefault("notes", []).append(
                        f"dropped_rule:{r.get('rule_id')}"
                    )
    ...
    def generate_rules_from_fact_sheet(self, fact_sheet: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Replace the model consolidation with deterministic lossless consolidation.

        This preserves counts and variants and avoids the over aggressive
        collapse that was seen when consolidation was done in the model.
        """
        rules_out = _lossless_consolidate_rules(fact_sheet)
        try:
            self.debug_dir.joinpath("rules_from_facts.json").write_text(
                json.dumps({"rules": rules_out}, indent=2, ensure_ascii=False),
                encoding="utf-8",
            )
        except Exception:
            pass
    ...
    def merge_sections(self, section_objs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Merge per section IRs into a single global IR.

        Steps:
        - build a fact sheet with variables and normalized rules
        - perform local lossless consolidation of rules
        - prune invalid conditions and preserve triage/ref variants
        - optionally call the merge model (STRICT_MERGE), else do a
          simple variable merge
        - resolve variable names and rewrite rules to canonical form
        - validate with pydantic when available
        """
        ...
        # Clean up conditions in consolidated rules.
        cleaned_rules: List[Dict[str, Any]] = []
        ...
        # Preserve ref and triage variants when needed.
        cleaned_rules, variant_notes = _preserve_ref_and_triage_variants(
            cleaned_rules, fact_sheet
        )
        ...
        merged: Dict[str, Any]
        if STRICT_MERGE:
            # Try the more sophisticated model assisted merge, but always
            # take our locally consolidated rules for the final rule set.
            try:
                ...
                # Fallback: simple variable merge, keep all consolidated rules.
                ...
                for v in (fact_sheet.get("variables") or []):
                    ...
                    if cur is None:
                        ...
                        syn = set(cur.get("synonyms") or []) | set(
                            v.get("synonyms") or []
                        )
                        refs = set(cur.get("refs") or []) | set(
                            v.get("refs") or []
                        )
            ...
            # Simple merge path used when STRICT_MERGE is False.
            variables: Dict[str, Dict[str, Any]] = {}
        ...
        # Append cleaning notes into qa.notes
        try:
        ...
        # Resolve variable names and rewrite rules accordingly.
        merged = _resolve_variables_snake_and_rewrite_rules(merged)

        # Build and apply canonical map.
        canon = _build_canonical_map(self.config, merged.get("variables", []))
        merged["canonical_map"] = canon
        merged["rules"] = [
            _rewrite_rule_to_canon(r, canon) for r in (merged.get("rules") or [])
        ]
        merged["rules"] = _dedupe_rule_ids(merged["rules"], prefix="merged")

        # Optional pydantic validation to catch any remaining schema drift.
        if PydanticAvailable:
            try:
                merged = IR.model_validate(merged).model_dump()
            except ValidationError as e:
                try:
                    self.debug_dir.joinpath(
                        "pydantic_ir_validation_error.txt"
    ...
    def generate_dmn_and_ask_plan(self, merged_ir: Dict[str, Any]) -> Tuple[str, Any]:
        """
        Call the DMN model to generate DMN XML and ASK_PLAN for the IR.

        Returns:
            dmn_xml (str), ask_plan (list or dict)
        """
        user = "RULES_JSON:\n" + json.dumps(merged_ir, ensure_ascii=False)
        text = self._chat_text(self.dmn_system, user, max_out=12000, model_key="dmn")

        try:
            Path(self.debug_dir / "dmn_ask_debug_last.txt").write_text(
                text, encoding="utf-8"
            )
        except Exception:
            pass
        ...
        # Prefer fenced blocks, but fall back to scanning the whole text.
        ...
        def _sanitize_dmn(xml: str) -> str:
            """
            Minor DMN cleanup so downstream XML tools do not fail on
            missing <dmn:text> wrappers or namespaces.
            """
            s = xml
            s = re.sub(
                r"(<dmn:outputEntry>\s*<dmn:text>)([^<]*?)(</dmn:outputEntry>)",
                r"\1\2</dmn:text>\3",
                s,
                flags=re.DOTALL,
            )
            ...
            s = re.sub(
                r"(<dmn:inputEntry>\s*<dmn:text>)([^<]*?)(</dmn:inputEntry>)",
                r"\1\2</dmn:text>\3",
                s,
                flags=re.DOTALL,
            )
        ...
        # Clean ASK_PLAN and drop references to unknown variables.
        try:
            known_vars = {
                v["name"]
                for v in merged_ir.get("variables", [])
            ...
            if isinstance(ask_plan, list):
                ...
                ask_plan = mutated
            if unknowns:
                merged_ir.setdefault("qa", {}).setdefault("notes", []).append(
                    f"ask_plan_unknowns_dropped:{sorted(list(unknowns))}"
                )
    ...
    def generate_bpmn(self, dmn_xml: str, ask_plan: Any) -> str:
        """
        Call the BPMN model to wrap the DMN and ASK_PLAN into a BPMN flow.
        """
        user = (
            "DMN:\n```xml\n"
            + dmn_xml
        ...
        if xml:
            ...
            xml = re.sub(
                r"</\s*definitions\s*>", "</bpmn:definitions>", xml, count=1
            )
            return _sanitize_bpmn(xml)
        try:
            dbg = text
            if len(dbg) > 24000:
                dbg = dbg[:16000] + "\n...<TRUNCATED>...\n" + dbg[-6000:]
            Path(self.debug_dir / "bpmn_debug_last.txt").write_text(
                dbg, encoding="utf-8"
            )
        except Exception:
            pass
    ...
    def audit_coverage(self, merged_ir: dict, dmn_xml: str) -> dict:
        """
        Estimate rule coverage of the DMN.

        Returns a dict with:
            - unmapped_rule_ids: list of rule_ids that appear uncovered
            - module_counts: counts of rules per DMN module
            - notes: free text notes for debugging

        If the model call fails or returns bad JSON, fall back to a cheap
        structural coverage computed from the DMN decision tables.
        """
        # Flatten rules into a lightweight view for the coverage model.
        rules_flat = _rules_flattened(merged_ir)
        user = (
            "RULES_JSON:\n"
            + json.dumps({"ir_flat": rules_flat}, ensure_ascii=False)
            + "\n\nDMN_XML:\n"
            + dmn_xml
        )

        # 1) Try the model route
        try:
            ...
            # 2) Fallback: derive minimal but useful coverage from the DMN itself
            try:
                tables = self.parse_dmn_decision_tables(dmn_xml)
                module_counts = {}
                for mod_name, t in tables.items():
                    module_counts[mod_name] = len(t.get("rows", []))

                # Minimal unmapped inference. Without explicit rule id
                # annotations in DMN there is no robust mapping, so we
                # return an empty list and rely on module_counts instead.
                unmapped_rule_ids: List[str] = []

                return {
                    "notes": [
                        "audit_coverage fallback used due to model JSON failure",
                ...
                # 3) Absolute last resort: return a stub so the pipeline can proceed
                return {
                    "notes": [
                        "audit_coverage double-fallback: DMN parse also failed",
                        f"first_exception: {e.__class__.__name__}: {str(e)}",
                        f"second_exception: {e2.__class__.__name__}: {str(e2)}",
                    ],
                    "module_counts": {},
                    "unmapped_rule_ids": [],
                }
    ...
    def export_xlsx_from_dmn(
        ...
        """
        Create an XLSForm file (orchestrator.xlsx) from IR and ASK_PLAN.

        If a template is provided it is respected and extended; otherwise
        a minimal survey, choices, and settings layout is created.
        """
        try:
        ...
        # Reset settings sheet so we do not accumulate stale rows across runs.
        ...
        ws_set.append(
            [
                ...
                "en",
                int(version_val),
                str(sms_keyword_default),
                str(sms_separator_default),
            ]
        )
        ...
        c_ix = {
            k: _col_ix(ws_choices, c_hdr, k)
            for k in ("list_name", "name", "label")
        }

        var_by_name = {
            (v.get("name") or "").strip(): v
        ...
        # Track already present choices so we do not duplicate across runs.
        ...
        def ensure_list(list_name: str, values: List[str]):
            for v in values or []:
                ...
                if key not in existing_choices:
                    ...
                    row[c_ix["name"]] = str(v)
                    row[c_ix["label"]] = (
                        re.sub(r"_+", " ", str(v)).strip().capitalize()
                    )
                    ws_choices.append(row)
        ...
        def cond_to_relevant(cond: str) -> Optional[str]:
            """
            Convert a simple FEEL comparison into an XLSForm relevant test.
            """
            s = str(cond or "").strip()
            m = re.match(
        ...
        # Collect relevant expressions for follow up questions.
        ...
        for blk in (ask_plan or []):
            ...
            append_row(
                {
                    "type": "begin group",
                    "name": f"{_to_snake(mod)}_group",
                    "label": re.sub(r"_+", " ", mod).title(),
                }
            )
            for q in (blk or {}).get("ask", []) or []:
                ...
                if qtype == "boolean":
                    ...
                    q_type_cell = "text"
                rel_expr = (
                    " or ".join(sorted(set(relevant_for[q])))
    ...
    def parse_dmn_decision_tables(self, dmn_xml: str) -> Dict[str, dict]:
        """
        Parse DMN decision tables into a simple dict representation.

        Returns a mapping:
            module_id -> { "inputs": [...], "outputs": [...], "rows": [
                { "conds": [...], "outs": [...] }
            ] }
        """
        ...
        for dec in root.findall(".//dmn:decision", _DMNN):
            ...
            if table is None:
                continue
            inputs = [
                _text_or(i.find("./dmn:inputExpression/dmn:text", _DMNN))
                for i in table.findall("./dmn:input", _DMNN)
            ]
            outputs = [
                (o.get("name") or "").strip()
                for o in table.findall("./dmn:output", _DMNN)
            ]
            rows = []
            for r in table.findall("./dmn:rule", _DMNN):
                conds = [
                    _text_or(e.find("./dmn:text", _DMNN))
                    for e in r.findall("./dmn:inputEntry", _DMNN)
                ]
                outs = [
                    _text_or(e.find("./dmn:text", _DMNN))
                    for e in r.findall("./dmn:outputEntry", _DMNN)
                ]
    ...
    def export_csvs_from_dmn(self, dmn_xml: str, out_dir: str) -> Dict[str, str]:
        """
        Export one CSV per DMN module with only the outputs.

        The CSVs are keyed by module name and used for simple orchestrator
        style lookup tables where needed.
        """
        ...
        def _normalize_page_ranges(ref: str) -> str:
            ...
            s = re.sub(
                r"[Pp]\s*(\d{1,4})\s*-\s*[Pp]?\s*(\d{1,4})", _swap_if_needed, s
            )
        ...
        for mod, t in tables.items():
            outs = [c for c in t["outputs"] if c] or [
                "triage",
                "danger_sign",
                "clinic_referral",
                "reason",
    ...
    def wire_decisions_into_xlsx(
        self,
        xlsx_path: str,
        dmn_xml: str,
        merged_ir: Dict[str, Any],
        media_id_prefix: str = "dmn_",
    ) -> None:
        """
        Append calculated fields and summary notes into the XLSForm.

        For each DMN module, we:
        - add calculate rows for triage, danger_sign, etc
        - add a note row with a human readable summary.
        """
        ...
        def map_var_token_to_xls(var_token: str) -> str:
            """
            Map a FEEL variable reference into an XLSForm calculate name.

            Example:
                decide_module_a.triage -> dmn_module_a_triage
            """
        ...
        def feel_to_xls_test(expr: str) -> Optional[str]:
            """
            Convert a simple FEEL comparison into an XLSIF style test.
            """
            s = (expr or "").strip()
            if s in ("", "-", "otherwise"):
                return None
            ...
            val = (rhs or "").strip()
            if val.lower() in ("true", "false"):
                rhs_x = "'true'" if lhs_is_calc else (
                    "'yes'" if val.lower() == "true" else "'no'"
                )
            elif re.match(r"^-?\d+(\.\d+)?$", val):
                rhs_x = val
            else:
    ...
    def build_orchestrator_folder(
        self,
        dmn_xml: str,
        ask_plan: List[Dict[str, Any]],  # noqa: D401
        merged_ir: Dict[str, Any],
        ...
        """
        Build a complete orchestrator folder for CHT from DMN and IR.

        This creates:
        - orchestrator.xlsx (form and decision wiring)
        - orchestrator.xml (placeholder XML wrapper)
        - orchestrator.properties.json (app metadata)
        - media/logo.png (either provided or a tiny transparent stub)
        """
        ...
        self.export_xlsx_from_dmn(
            merged_ir, ask_plan, str(xlsx_path), template_xlsx_path
        )
        ...
        if logo_path and Path(logo_path).exists():
            ...
            default_logo = Path(__file__).with_name(
                "205a491d-21bb-46eb-be6e-6e5279e4156b.png"
            )
    ...
    def extract_sections_from_pdf(
        self, pdf_path: str, max_chars: int = 4000
    ) -> List[Tuple[str, str]]:
        """
        Multi backend PDF text extraction.

        Strategy:
        1) pypdf (fast, no extra deps)
        2) pdfminer layout (richer layout, placeholder)
        3) pdfminer basic (placeholder)
        4) OCR (placeholder)

        The first backend that yields any non empty text wins.
        """
        ...
        if _have_pdfminer:
            ...
            if sum(1 for _, t in pm_layout if (t or "").strip()) > 0:
                chunks = _split_into_sections_by_headings(
                    pm_layout, max_chars=max_chars
                )
                if not chunks:
                    chunks = _chunk_pages_len_only(
                        pm_layout, max_chars=max_chars
                    )
                try:
                    logp.write_text(
                        "Used pdfminer layout extraction\n", encoding="utf-8"
                    )
                except Exception:
                    pass
                return chunks
        ...
        if _have_pdfminer:
            ...
            if sum(1 for _, t in pm_pages if (t or "").strip()) > 0:
                ...
                try:
                    logp.write_text(
                        "Used pdfminer basic extraction\n", encoding="utf-8"
                    )
                except Exception:
                    pass
                return chunks
        ...
        if _have_ocr:
            ...
            if sum(1 for _, t in ocr_pages if (t or "").strip()) > 0:
                ...
                try:
                    logp.write_text(
                        "Used OCR extraction (pytesseract + pdf2image)\n",
                        encoding="utf-8",
                    )
                except Exception:
                    pass
        ...
        try:
            logp.write_text(
                "No text extracted by any strategy\n", encoding="utf-8"
            )
        except Exception:
            pass
        return []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--merged", required=False, help="Existing merged_ir.json path"
    )
    parser.add_argument(
        "--ask", required=False, help="Existing ask_plan.json path"
    )
    parser.add_argument(
        "--xlsx-out", default="forms/app/orchestrator.xlsx"
    )
    parser.add_argument(
        "--media-dir", default="forms/app/orchestrator-media"
    )
    ...
    if args.pdf:
        ...
        if args.ask and Path(args.ask).exists():
            ...
            ask_plan = (
                raw.get("ASK_PLAN")
                if isinstance(raw, dict) and "ASK_PLAN" in raw
                else raw
            )
        else:
            ask_plan = [
        ...
        print(
            "Supply either --pdf OR both --dmn and --merged", file=sys.stderr
        )
        sys.exit(2)
